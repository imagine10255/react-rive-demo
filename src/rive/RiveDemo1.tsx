...
const RiveDemo1 = () => {
  const STATE_MACHINE_NAME = 'State Machine '; // 差一個空白有差，以為只是命名誒
  const INPUT_NAME = 'Level';
  const { rive, RiveComponent: RiveComponentTouch } = useRive({
    //src: 'https://rive-app.github.io/rive-react/skills.riv',
    src: '/rive/demo1_skills.riv',
  ...
  return (
    ...
    <div style={{ width: '400px', height: '400px' }}>
      ...
      <div className="btn-group">
        Choose a level:
        <button type="button" onClick={() => (levelInput.value = 0)}>
          0
        </button>
        <button type="button" onClick={() => (levelInput.value = 1)}>
          1
        </button>
        <button type="button" onClick={() => (levelInput.value = 2)}>
          2
        </button>
      </div>
    </div>
